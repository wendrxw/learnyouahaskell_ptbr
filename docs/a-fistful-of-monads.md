# Por um Punhado de Mônadas

Quando falamos pela primeira vez sobre functores, vimos que eles eram um conceito útil para valores que podem ser mapeados.
Então, levamos esse conceito um passo adiante introduzindo functores aplicativos, que nos permitem ver valores de certos tipos de dados como valores com contextos e usar funções normais nesses valores, preservando o significado desses contextos.

Neste capítulo, aprenderemos sobre mônadas, que são apenas functores aplicativos anabolizados, assim como functores aplicativos são apenas functores anabolizados.

![more cool than u](assets/images/a-fistful-of-monads/smugpig.png){.right width=307 height=168}

Quando começamos com functores, vimos que é possível mapear funções sobre vários tipos de dados.
...
Então vimos uma possível melhoria dos functores e dissemos, ei, e se essa função `a -> b` já estiver embrulhada dentro de um valor functor?
...
Como dissemos, um valor aplicativo pode ser visto como um valor com um contexto adicionado.
...
Ah, legal, então agora que os tratamos como valores aplicativos, valores `Maybe a` representam computações que podem ter falhado, valores `[a]` representam computações que têm vários resultados (computações não determinísticas), valores `IO a` representam valores que têm efeitos colaterais, etc.

Mônadas são uma extensão natural de functores aplicativos e com elas estamos preocupados com isso: se você tem um valor com um contexto, `m a`, como você aplica a ele uma função que pega um `a` normal e retorna um valor com um contexto?
...
Esta é a principal questão com a qual nos preocuparemos ao lidar com mônadas.
Escrevemos `m a` em vez de `f a` porque o `m` significa `Monad`, mas mônadas são apenas functores aplicativos que suportam `>>=`.
...
Agora que temos uma vaga ideia sobre o que são mônadas, vamos ver se podemos tornar essa ideia um pouco menos vaga.

Para surpresa de ninguém, `Maybe` é uma mônada, então vamos explorá-la um pouco mais e ver se podemos combiná-la com o que sabemos sobre mônadas.

::: {.hintbox}
Certifique-se de entender [aplicativos](functors-applicative-functors-and-monoids.html#applicative-functors) neste ponto.
É bom se você tiver uma noção de como as várias instâncias `Applicative` funcionam e que tipo de computações elas representam, porque mônadas nada mais são do que pegar nosso conhecimento aplicativo existente e atualizá-lo.
:::
...
Como um functor aplicativo, ele funciona de forma semelhante.
No entanto, aplicativos também têm a função embrulhada.
`Maybe` é um functor aplicativo de tal forma que quando usamos `<*>` para aplicar uma função dentro de um `Maybe` a um valor que está dentro de um `Maybe`, ambos têm que ser valores `Just` para que o resultado seja um valor `Just`, caso contrário, o resultado é `Nothing`.
...
Quando usamos o estilo aplicativo para ter funções normais agindo sobre valores `Maybe`, é semelhante.
...
Para descobrir como ele faz isso, podemos usar a intuição que temos de `Maybe` sendo um functor aplicativo.
...
Se pensarmos sobre como `Maybe` age como um functor aplicativo, responder a isso é muito fácil.
...
Isso é muito semelhante a quando usamos `Maybe` como um aplicativo e obtivemos um resultado `Nothing` se algo ali dentro fosse um `Nothing`.
...
Pode parecer que functores aplicativos são mais fortes que mônadas, já que functores aplicativos nos permitem pegar uma função normal e fazê-la operar em valores com contextos.
Veremos que mônadas podem fazer isso também porque são uma atualização de functores aplicativos, e que elas também podem fazer algumas coisas legais que functores aplicativos não podem.

Voltaremos ao `Maybe` em um minuto, mas primeiro, vamos verificar a typeclass que pertence às mônadas.

## A typeclass Monad {#the-monad-type-class}

Assim como functores têm a typeclass `Functor` e functores aplicativos têm a typeclass `Applicative`, mônadas vêm com sua própria typeclass: `Monad`!
Uau, quem diria?
...
Mas espere, não dissemos que mônadas são apenas functores aplicativos anabolizados?
Não deveria haver uma restrição de classe lá na linha de `class (Applicative m) => Monad m where` para que um tipo tenha que ser um functor aplicativo primeiro antes de poder ser feito uma mônada?
Bem, deveria, mas quando Haskell foi feito, não ocorreu às pessoas que functores aplicativos são um bom ajuste para Haskell, então eles não estavam lá.
Mas fique tranquilo, toda mônada é um functor aplicativo, mesmo que a declaração da classe `Monad` não diga isso.
...
Em outras palavras, ela pega algo e o embrulha em uma mônada.
...
A próxima função é `>>=`, ou bind.
É como a aplicação de função, só que em vez de pegar um valor normal e alimentá-lo a uma função normal, ela pega um valor monádico (ou seja, um valor com um contexto) e o alimenta a uma função que pega um valor normal, mas retorna um valor monádico.
...
Em vez disso, ela é usada pelo Haskell para permitir falha em uma construção sintática especial para mônadas que encontraremos mais tarde.
...
Podemos brincar com `Maybe` como uma mônada:
...
Com isso, agora podemos encadear pousos que podem falhar porque `>>=` nos permite alimentar um valor monádico a uma função que pega um normal.
...
Mas vamos tentar isso agora usando aplicação monádica (`>>=`) em vez de aplicação normal:
...
Não poderíamos ter conseguido isso apenas usando `Maybe` como um aplicativo.
Se você tentar, ficará preso, porque functores aplicativos não permitem que os valores aplicativos interajam muito entre si.
Eles podem, no máximo, ser usados como parâmetros para uma função usando o estilo aplicativo.
Os operadores aplicativos buscarão seus resultados e os alimentarão para a função de uma maneira apropriada para cada aplicativo e então montarão o valor aplicativo final, mas não há tanta interação acontecendo entre eles.
...
Em vez de fazer funções que ignoram sua entrada e apenas retornam um valor monádico predeterminado, podemos usar a função `>>`, cuja implementação padrão é esta:
...
Com mônadas, no entanto, seu contexto e significado também devem ser considerados.
...
Converter essa monstruosidade em uma cadeia organizada de aplicações monádicas com `>>=` é um exemplo clássico de como a mônada `Maybe` nos poupa muito tempo quando temos que fazer sucessivamente computações que são baseadas em computações que podem ter falhado.
...
## Notação do {#do-notation}

Mônadas em Haskell são tão úteis que ganharam sua própria sintaxe especial chamada notação `do`.
Já encontramos a notação `do` quando estávamos fazendo E/S e lá dissemos que era para colar várias ações de E/S em uma.
Bem, como se vê, a notação `do` não é apenas para `IO`, mas pode ser usada para qualquer mônada.
Seu princípio ainda é o mesmo: colar valores monádicos em sequência.
Vamos dar uma olhada em como a notação `do` funciona e por que é útil.

Considere este exemplo familiar de aplicação monádica:
...
Já vimos isso.
Alimentar um valor monádico a uma função que retorna um, nada demais.
Observe como quando fazemos isso, `x` se torna `3` dentro do lambda.
Uma vez que estamos dentro desse lambda, é apenas um valor normal, em vez de um valor monádico.
...
A principal diferença entre esses dois é que os valores no primeiro exemplo são monádicos.
...
Então isso é meio como atribuir valores a variáveis em expressões `let`, só que os valores em questão são valores monádicos.
...
Para nos salvar de escrever todos esses lambdas irritantes, Haskell nos dá a notação `do`.
...
É importante lembrar que expressões `do` são apenas uma sintaxe diferente para encadear valores monádicos.

Em uma expressão `do`, cada linha é um valor monádico.
Para inspecionar seu resultado, usamos `<-`.
Se tivermos um `Maybe String` e o vincularmos com `<-` a uma variável, essa variável será uma `String`, assim como quando usamos `>>=` para alimentar valores monádicos a lambdas.
O último valor monádico em uma expressão `do`, como `Just (show x ++ y)` aqui, não pode ser usado com `<-` para vincular seu resultado, porque isso não faria sentido se traduzíssemos a expressão `do` de volta para uma cadeia de aplicações `>>=`.
Em vez disso, seu resultado é o resultado de todo o valor monádico colado, levando em consideração a possível falha de quaisquer anteriores.
...
Se compararmos esses dois, é fácil ver por que o resultado de todo o valor monádico é o resultado do último valor monádico na expressão `do` com todos os anteriores encadeados nele.

A rotina de nosso equilibrista também pode ser expressa com a notação `do`.
...
Com expressões `do`, no entanto, cada linha deve apresentar um valor monádico.
...
Como expressões `do` são escritas linha por linha, elas podem parecer código imperativo para algumas pessoas.
...
Novamente, vamos dar uma olhada em como este trecho de código seria se não tivéssemos usado os aspectos monádicos de `Maybe`:
...
Se quisermos jogar uma casca de banana para Pierre na notação `do`, podemos fazer o seguinte:
...
Quando escrevemos uma linha na notação `do` sem vincular o valor monádico com `<-`, é exatamente como colocar `>>` após o valor monádico cujo resultado queremos ignorar.
Nós sequenciamos o valor monádico, mas ignoramos seu resultado porque não nos importamos com o que ele é e é mais bonito do que escrever `_ <- Nothing`, que é equivalente ao acima.

Quando usar a notação `do` e quando usar explicitamente `>>=` depende de você.
...
Com a notação `do`, tivemos que escrever especificamente em qual vara os pássaros estão pousando, mas toda vez usamos aquela que veio logo antes.
Mas ainda assim, isso nos deu alguma visão sobre a notação `do`.

Na notação `do`, quando vinculamos valores monádicos a nomes, podemos utilizar pattern matching, assim como em expressões `let` e parâmetros de função.
Aqui está um exemplo de pattern matching em uma expressão `do`:
...
Quando o pattern matching falha em uma expressão `do`, a função `fail` é chamada.
Ela faz parte da typeclass `Monad` e permite que o pattern matching falho resulte em uma falha no contexto da mônada atual em vez de fazer nosso programa travar.
...
Então, por padrão, ela faz nosso programa travar, mas mônadas que incorporam um contexto de possível falha (como `Maybe`) geralmente a implementam por conta própria.
...
O pattern matching falho causou uma falha dentro do contexto de nossa mônada em vez de causar uma falha em todo o programa, o que é bem legal.

## A mônada de lista {#the-list-monad}
...
Nesta seção, vamos dar uma olhada em como usar os aspectos monádicos de listas para trazer não-determinismo ao nosso código de uma maneira clara e legível.

Já falamos sobre como listas representam valores não-determinísticos quando são usadas como aplicativos.
...
Usar listas como functores aplicativos mostra bem esse não-determinismo:
...
Esse contexto de não-determinismo se traduz para mônadas muito bem.
...
`>>=` é sobre pegar um valor com um contexto (um valor monádico) e alimentá-lo a uma função que pega um valor normal e retorna um que tem contexto.
...
Quando usamos `>>=` com `Maybe`, o valor monádico foi alimentado na função enquanto cuidava de possíveis falhas.
...
Aqui está a expressão anterior reescrita na notação `do`:
...
Assim como com `Maybe`, estamos extraindo os elementos dos valores monádicos e tratando-os como valores normais e `>>=` cuida do contexto para nós.
O contexto neste caso é o não-determinismo.

Usar listas com notação `do` realmente me lembra de algo que vimos antes.
...
Em nosso exemplo de notação `do`, `n` tornou-se cada resultado de `[1,2]` e para cada resultado, `ch` recebeu um resultado de `['a','b']` e então a linha final colocou `(n,ch)` em um contexto padrão (uma lista unitária) para apresentá-lo como o resultado sem introduzir qualquer não-determinismo adicional.
Nesta compreensão de lista, a mesma coisa aconteceu, só que não tivemos que escrever `return` no final para apresentar `(n,ch)` como o resultado porque a parte de saída de uma compreensão de lista fez isso por nós.

De fato, compreensões de lista são apenas açúcar sintático para usar listas como mônadas.
No final, compreensões de lista e listas em notação `do` se traduzem em usar `>>=` para fazer computações que apresentam não-determinismo.

Compreensões de lista nos permitem filtrar nossa saída.
...
Muito inteligente.
Para ver como a filtragem em compreensões de lista se traduz para a mônada de lista, temos que verificar a função `guard` e a typeclass `MonadPlus`.
A typeclass `MonadPlus` é para mônadas que também podem agir como monóides.
...
Como listas são monóides, bem como mônadas, elas podem ser feitas uma instância desta typeclass:
...
Caso contrário, ela faz um valor monádico de falha.
...
Na mônada de lista, nós a usamos para filtrar computações não-determinísticas.
Observe:
...
Aqui está o exemplo anterior reescrito na notação `do`:
...
Aqui está isso novamente na forma de uma compreensão de lista:
...
Portanto, a filtragem em compreensões de lista é o mesmo que usar `guard`.
...
Esta função também pode ser escrita sem o uso de listas como uma mônada, mas fizemos isso aqui apenas por diversão.
...
O exemplo acima sem a notação `do`:
...
Assim como functores aplicativos, e functores antes deles, mônadas vêm com algumas leis que todas as instâncias de mônadas devem respeitar.
Só porque algo é feito uma instância da typeclass `Monad` não significa que é uma mônada, significa apenas que foi feito uma instância de uma typeclass.
Para que um tipo seja verdadeiramente uma mônada, as leis das mônadas devem valer para esse tipo.
...
Ele não pode verificar se as leis das mônadas valem para um tipo, no entanto, então, se estamos fazendo uma nova instância da typeclass `Monad`, temos que estar razoavelmente certos de que tudo está bem com as leis das mônadas para esse tipo.
Podemos confiar nos tipos que vêm com a biblioteca padrão para satisfazer as leis, mas mais tarde, quando formos fazer nossas próprias mônadas, teremos que verificar manualmente se as leis valem.
Mas não se preocupe, elas não são complicadas.

### Identidade à esquerda {#left-identity}

A primeira lei das mônadas afirma que se pegarmos um valor, o colocarmos em um contexto padrão com `return` e então o alimentarmos a uma função usando `>>=`, é o mesmo que apenas pegar o valor e aplicar a função a ele.
Para colocar formalmente:

* `return x >>= f`{.label .law} é a mesma maldita coisa que `f x`{.label .law}

Se você olhar para valores monádicos como valores com um contexto e `return` como pegar um valor e colocá-lo em um contexto mínimo padrão que ainda apresenta esse valor como seu resultado, faz sentido, porque se esse contexto é realmente mínimo, alimentar esse valor monádico a uma função não deve ser muito diferente de apenas aplicar a função ao valor normal, e de fato não é diferente de forma alguma.

Para a mônada `Maybe`, `return` é definida como `Just`.
A mônada `Maybe` é toda sobre possível falha, e se temos um valor e queremos colocá-lo em tal contexto, faz sentido que o tratemos como uma computação bem-sucedida porque, bem, sabemos qual é o valor.
...
Para a mônada de lista, `return` coloca algo em uma lista unitária.
...
### Identidade à direita {#right-identity}

A segunda lei afirma que se temos um valor monádico e usamos `>>=` para alimentá-lo a `return`, o resultado é nosso valor monádico original.
Formalmente:
...
Quando alimentamos valores monádicos a funções usando `>>=`, essas funções pegam valores normais e retornam valores monádicos.
...
Aqui está um teste para algumas mônadas:
...
É uma função importante para transformar valores normais em valores monádicos e não seria bom se o valor monádico que ela produzisse fizesse um monte de outras coisas.

### Associatividade {#associativity}

A lei final das mônadas diz que quando temos uma cadeia de aplicações de funções monádicas com `>>=`, não deve importar como elas são aninhadas.
Escrito formalmente:

* Fazer `(m >>= f) >>= g`{.label .law} é exatamente como fazer `m >>= (\x -> f x >>= g)`{.label .law}

Hmmm, agora o que está acontecendo aqui?
Temos um valor monádico `m` e duas funções monádicas `f` e `g`.
Quando estamos fazendo `(m >>= f) >>= g`, estamos alimentando `m` a `f`, o que resulta em um valor monádico.
Então, alimentamos esse valor monádico a `g`.
Na expressão `m >>= (\x -> f x >>= g)`, pegamos um valor monádico e o alimentamos a uma função que alimenta o resultado de `f x` a `g`.
...
Começamos com `Just (0,0)` e então vinculamos esse valor à próxima função monádica, `landRight 2`.
O resultado disso foi outro valor monádico que foi vinculado à próxima função monádica, e assim por diante.
...
Então não importa como você aninha a alimentação de valores para funções monádicas, o que importa é o significado delas.
...
Agora, e se essas duas funções fossem monádicas, ou seja, e se os valores que elas retornassem fossem valores monádicos?
Se tivéssemos uma função do tipo `a -> m b`, não poderíamos simplesmente passar seu resultado para uma função do tipo `b -> m c`, porque essa função aceita um `b` normal, não um monádico.
No entanto, poderíamos usar `>>=` para fazer isso acontecer.
Então, usando `>>=`, podemos compor duas funções monádicas:
...
Então agora podemos compor duas funções monádicas:
...
Esta é apenas outra maneira de dizer que para mônadas, o aninhamento de operações não deve importar.

Se traduzirmos as duas primeiras leis para usar `<=<`, então a lei da identidade à esquerda afirma que para toda função monádica `f`, `f <=< return`{.label .law} é o mesmo que escrever apenas `f`{.label .law} e a lei da identidade à direita diz que `return <=< f`{.label .law} também não é diferente de `f`{.label .law}.
...
Neste capítulo, demos uma olhada no básico das mônadas e aprendemos como a mônada `Maybe` e a mônada de lista funcionam.
No próximo capítulo, daremos uma olhada em um monte de outras mônadas legais e também aprenderemos como fazer as nossas próprias.
